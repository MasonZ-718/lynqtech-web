...
```
   ...
   Your friend is a
     Loop Thinker
   and you can't stop replaying decisions  ← secondary, if present
...
1. **Friend-view branding:** does the friend view show Lynq Tech / ReAnchor branding, or is it framed purely around "your friend"? (Proposal: tiny brand mark at bottom, same as identity card.)
2. **Share analytics:** do we want to emit a `cta_source` value for share-button clicks (e.g., `share_native` / `share_copy`) so we can measure virality? (Proposal: yes, cheap to add; write a row to `quiz_results` with the share source when the button is pressed if the user has already submitted an email, otherwise skip.)
